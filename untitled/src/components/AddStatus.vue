    ...
    <div class="form-container">
      ...
      <form @submit.prevent="created">
        <div class="form-group">
          ...
          <input type="text" id="statusName" name="statusName" class="form-control-large" v-model="statusName">
        </div>
        <div class="form-group">
          <button class="btn-submit">Add Status</button>
        </div>
      </form>
    </div>
  </div>
  ...
  methods: {
    async created() {
      try {
        ...
        const resp = await axios.post("http://localhost:8080/product/status", payload);
